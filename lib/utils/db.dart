...
//    Future<DocumentReference> future = userCollection.add(task.convertToMap());
    Future<DocumentReference> future = dbReference.collection('todo').add(task.convertToMap());
    return future;
  }
  ...
  static Future<void> updateTask(String id, Task task) {
    CollectionReference userCollection = dbReference.collection('todo');
    Future<void> future =  userCollection.document(id).updateData(task.convertToMap());
    return future;
  }


  static Future<DocumentReference> addToDo(ToDo todoobj){
    Future<DocumentReference> future = dbReference.collection('todo').add(todoobj.convertToMap());
    return future;
  }
}